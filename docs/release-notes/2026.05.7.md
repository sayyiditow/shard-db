...
> **[Erratum, 2026-09]** The `add-field` claim above is wrong: `add-field`
> has never auto-bitmapped — it adds no indexes at all. Bitmap on bool
> fields was only ever implicit at *create-object* (undeclared bool/enum
> fields got `IT_BITMAP`), and that auto-default itself was removed in a
> later release — indexes are created only when declared.
