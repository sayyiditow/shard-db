...
| `INDEX_BUILD_BUDGET_MB` | `1024` | Peak per-pass memory budget for `reindex` and multi-field `add-index`. Floor 64 MB. Multi-field builds group fields into passes that fit this cap; an oversized single field still runs alone. See [Tuning → INDEX_BUILD_BUDGET_MB](../operations/tuning.md). |
...
| `BULK_COMMIT_WINDOW` | `4096` | Record count per synchronous durability window (M/A/I/K/T/C marker cycle) during bulk-insert/bulk-update/bulk-delete. Integer in `[16, 16384]`. Smaller windows bound the amount of work replayed after a crash mid-window; larger windows amortize marker fsync overhead over more records. Default 4096 since 2026.09.x (was 1024): with per-window batched index/kf syncing, larger windows mainly amortize marker publication costs. Shard wrlock hold per window grows proportionally. |
