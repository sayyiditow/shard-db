...
1. Takes the object's **write lock** (`objlock_wrlock`).
2. Refuses immediately if `storage_version != 2` (upgrade to 2026.05.4 and run that historical release's `./migrate` first; 2026.09.1 does not convert legacy objects).
