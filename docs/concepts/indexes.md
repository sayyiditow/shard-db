...
Older formats (`BTRE`, `BTRF`, `BTRG`) are rejected at open with a clear error and require an explicit reindex. 2026.08.2 never rebuilds indexes during startup compatibility checks. From pre-2026.05.1 installs, install 2026.05.4 first and run that historical release's `./migrate` to convert v1 objects to slotcask, then upgrade to 2026.05.5+.
