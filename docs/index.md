...
- **Three index types** — pick one per field:
    ...
    - **Bitmap** (opt-in via `field:bitmap` / `field:bitmap(N)` — for `bool`, `enum`, and low-cardinality varchar; a declared *bare* bool/enum name also promotes to bitmap) — one dense bit per slot, per-distinct-value, at `indexes/<field>/NNN.bm`. Popcount fast paths for `eq`/`in`/`neq`/`not_in`; dict-scan for every other op. 1:1 shard layout with data.
