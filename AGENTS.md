...
- **Dynamic-safety tooling for this repo (CORE-PROCESS.md's "Definition of done" gate):** ASan+UBSan and TSan, via `BUILD_MODE`. Any diff touching locks, shared/cached state (kfcache, segcache, bitmap cache, btree cache), object lifetimes, or background threads must be run locally under both before being called done — not deferred to CI. This workspace supports normal default all-core parallelism for both sanitizers; do not add `--jobs` unless a newly demonstrated harness limitation requires it. Run each full suite three consecutive times:
  ...
  - `BUILD_MODE=tsan SKIP_TESTS=1 ./build.sh`, then three fresh runs of `TSAN_OPTIONS="second_deadlock_stack=1:print_stacktrace=1:suppressions=$(pwd)/.tsan.supp" ./build/bin/shard-db-test run-all`.
...
- **Indexed-write crash safety**: indexed mutations persist a durable commit-intent marker (KFM2 batch marker for both single and bulk writes) before secondary-index apply, published via `bulk_publish_window_marker_locked`. Recovery is forward-replay-only: a crash at any point after the marker is durable causes startup to forward-replay it and apply the M/A/I/K/T/C window to completion, never roll it back. There is no abort-sidecar path — a prior release (2026.08.1) had one, but this design was removed; corrupt or unparseable marker evidence fails closed.
