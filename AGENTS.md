...
- **Dynamic-safety tooling for this repo (CORE-PROCESS.md's "Definition of done" gate):** ASan+UBSan and TSan, via `BUILD_MODE`. Any diff touching locks, shared/cached state (kfcache, segcache, bitmap cache, btree cache), object lifetimes, or background threads must be run locally under both before being called done — not deferred to CI. ASan+UBSan uses the normal default all-core parallelism. TSan is resource-constrained on this workstation and must use `--jobs 2` for every full-suite invocation. Run each full suite three consecutive times:
  ...
  - `BUILD_MODE=tsan SKIP_TESTS=1 ./build.sh`, then three fresh runs of `TSAN_OPTIONS="second_deadlock_stack=1:print_stacktrace=1" ./build/bin/shard-db-test run-all --jobs 2` (no suppressions file — `.tsan.supp` was eliminated 2026-08-30; every prior finding is fixed or restructured).
...
- **Indexed-write crash safety**: indexed mutations persist a durable commit-intent marker (KFM2 **V2** batch marker — 16-byte header + one 32-byte `KfMarkerSlot` per record, exact-size validated, `1 ≤ count ≤ 16384`; replay re-derives record bytes from the named segment records) before secondary-index apply. Bulk commands run as deferred requests (one IO-pool pipeline task per touched shard under per-kf-shard writer gates — a task holds only its own gate and never waits on another while holding it — see `docs/concepts/concurrency.md`); single writes keep per-window immediate durability. Per-file durability syncs coalesce across concurrent pipelines. Recovery is forward-replay-only: a crash at any point after the marker is durable causes startup to forward-replay it and apply the window to completion, never roll it back. Marker identity is the exact on-disk path (`MarkerRef`, nonce-bearing names); corrupt, unparseable, or pre-V2 marker evidence fails closed.
...
**Indexed batch ingest (pattern guidance)**: at 1M / 5×200K with 14 indexes, pre-existing-indexes parallel beat load-then-index by ~1.3–1.7× on the pre-2026.09 tree (435 k/sec vs 331 k/sec and 263 k/sec — those absolutes predate the durability rework and will read lower today; the *ordering* is structural and unchanged). The earlier "load-then-index always wins" framing came from bash and is reversed in C bench at this scale. **Crossover rule of thumb:** at `R ≈ N/200K`, pre-existing-indexes parallel wins for `R ≤ ~10`, load-then-index wins for `R ≥ ~20` (per-(field, shard) merge cost scales `O(R²)`). Re-bench at your scale; both patterns are valid.
