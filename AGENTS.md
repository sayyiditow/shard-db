...
- **Dynamic-safety tooling for this repo (CORE-PROCESS.md's "Definition of done" gate):** ASan+UBSan and TSan, via `BUILD_MODE`. Any diff touching locks, shared/cached state (kfcache, segcache, bitmap cache, btree cache), object lifetimes, or background threads must be run locally under both before being called done — not deferred to CI. This workspace supports normal default all-core parallelism for both sanitizers; do not add `--jobs` unless a newly demonstrated harness limitation requires it. Run each full suite three consecutive times:
  - `BUILD_MODE=asan SKIP_TESTS=1 ./build.sh`, then three fresh runs of `ASAN_OPTIONS="halt_on_error=0:detect_leaks=1:abort_on_error=0:print_stacktrace=1" ./build/bin/shard-db-test run-all`.
  - `BUILD_MODE=tsan SKIP_TESTS=1 ./build.sh`, then three fresh runs of `TSAN_OPTIONS="halt_on_error=0:second_deadlock_stack=1:print_stacktrace=1:suppressions=$(pwd)/.tsan.supp" ./build/bin/shard-db-test run-all`.
...
- **Indexed-write crash safety**: indexed mutations persist a commit-intent marker before secondary-index apply. A pre-publication crash forward-replays that marker; a post-marker index failure first persists a matching abort sidecar, then applies the inverse index diff and rejects the mutation. Recovery never forward-replays a marker paired with a valid abort sidecar. Marker cleanup is synced before sidecar cleanup, and corrupt/mismatched evidence fails closed.
