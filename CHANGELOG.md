...
- **[2026.08.1](docs/release-notes/2026.08.1.md)** — **Automatic startup migration**: replaces the standalone `./migrate` binary with a version-gated `$DB_ROOT/.version` check on both the daemon and embedded (`shard_db_open()`) paths; runs a full secondary-index reindex in-process when the on-disk version is older, refuses to start on a newer (downgrade) version. **ACID hardening**: atomic indexed writes across crash boundaries, crash-safe object rebuild (`RebuildTxn`), atomic partial updates and index publication. **New**: opt-in auto-reshard. **Concurrency fixes**: read-path UAF, btree-cache eviction race, lock-order inversions, rebuild-rename cache staleness. **NQL**: quoted-literal escaping (breaking: unterminated literals now error), `--filter` on aggregate, lock-free reads. **Removed**: dead v1 ucache/shard-file engine, `rebuild-kf`. **Fixes**: JSON-escaped varchar handling across storage/indexing/criteria/cursor/CSV paths, a `get`+`fields` v2 dispatch bug, a `decode_field` stack overflow, a `count` error-swallowing bug, writer-preferring per-file cache rwlocks, plus input-validation hardening across the criteria/NQL parsers and server dispatch. No on-disk format changes.
