...
# Sample wchan (kernel function where blocked) at 5 points during bench
(
    for i in 1 2 3 4 5; do
        sleep 0.5
        echo "---- sample $i ----"
        for tid in /proc/$SERVER_PID/task/*; do
            st=$(awk '/^State:/ {print $2}' $tid/status 2>/dev/null)
            wc=$(cat $tid/wchan 2>/dev/null)
            [ -z "$wc" ] && wc="(running)"
            echo "$st $wc"
