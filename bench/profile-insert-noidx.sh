#!/bin/bash
# Profile parallel CSV bulk-insert with NO indexes — isolates the non-index hot path.
cd "$(dirname "$0")/.."
...
kill -0 $SERVER_PID 2>/dev/null || { echo "abort"; exit 1; }

echo ""
echo "=== perf stat — parallel CSV × $CONNS conns, NO indexes ==="
recreate_object_noidx
( fire_parallel_inserts ) &
WORK_PID=$!
...
echo ""
echo "=== perf record — same (no indexes) ==="
recreate_object_noidx
( fire_parallel_inserts ) &
WORK_PID=$!
perf record -F 997 -g -p $SERVER_PID -o /tmp/perf_noidx.data -- sleep $PROFILE_SECS 2>&1 | tail -3
wait $WORK_PID

echo ""
echo "--- top by self% ---"
perf report -i /tmp/perf_noidx.data --stdio --no-children -n 2>/dev/null \
  | awk '/^# Overhead/{hdr=1} hdr{print}' | head -40

echo ""
echo "--- top by inclusive% ---"
perf report -i /tmp/perf_noidx.data --stdio -n 2>/dev/null \
  | awk '/^# Children/{hdr=1} hdr{print}' | head -40
