...
# Clients hitting the cap get {"error":"server at capacity"} immediately and
# should retry. Lower this if you want to leave more headroom for OS page
# cache; raise it on a beefy box with many short-lived queries.
...
# Per-query intermediate buffer. With MAX_CONCURRENT_QUERIES bounding fan-in,
# worst-case RAM stays predictable. 256 MB is sized for ~16 concurrent queries
# on a 16 GB host; auto-tunes upward on big-RAM hosts with low slot counts.
